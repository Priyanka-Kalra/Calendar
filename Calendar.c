...
//Function for determining the day of given date.
...
int determine_day(int date, int month, int year)
{
    ...
    left2 = rem - left;   //left2 stores no. of years <100.
    rem2 = left2 / 4;     //rem2 stores no. of leap years.
    ...
    {
        oddDays3 = oddDays3 + (days_in_month[i] % 7);
    }
    ...
    //Calculatiion of total odd days altogether
    int oddDaysT = oddDaysY + oddDaysM; //for both of months and years
    if (oddDaysT > 6)
    {
        oddDaysT %= 7;
    }
...
{
    {
        ...
        for (int month = 1; month <= 12; month++)
        {
            ...
            for (weekDay = 1; weekDay < startingDay; weekDay++)
                printf("   \t");
...
S:
    ...
    printf("\nEnter your choice::");
    printf("\nFor Checking day of the entered date press 1::");
    printf("\nFor printing calender of the entered year press 2::");
    scanf("%d", &choice);

    if (choice == 1) //for determing day of given date
    {
        int m = determine_day(date, month, year);
        switch (m)
        {
        case 0:
    ...
    {
        printf("Thankyou for using this program, Bye!");
    }
